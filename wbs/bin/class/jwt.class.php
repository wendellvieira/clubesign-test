<?php 
	     ...
	     */
		    public function decode($token) {
		        if (substr_count($token, '.') < 2) {
		            throw new Exception('Invalid token: Incomplete segments', static::ERROR_TOKEN_INVALID);
		        }
		        $token = explode('.', $token, 3);
		        $this->validateHeader((array) $this->urlSafeDecode($token[0]));
		        // Validate signature.
		        if (!$this->verify($token[0] . '.' . $token[1], $token[2])) {
		            throw new Exception('Invalid token: Signature failed', static::ERROR_SIGNATURE_FAILED);
		        }
     ...
     *
     * @throws Exception When JSON encode fails.
     *
     * @return string
     */
